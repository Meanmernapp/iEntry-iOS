//
//  getAllInvitationsAgainstEventModel.swift
//  iEntry
//
...
import Foundation
struct getAllInvitationsAgainstEventModel : Codable {
    let code : Int?
    let data : [getAllInvitationsAgainstEventModelData]?
    let success : Bool?
    let message : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        data = try values.decodeIfPresent([getAllInvitationsAgainstEventModelData].self, forKey: .data)
...
struct getAllInvitationsAgainstEventModelData : Codable {
    let id : String?
    //let manager : Manager?
    let guest : Guest?
    let event : Event?
